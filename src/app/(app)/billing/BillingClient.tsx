  ...
  return (
    ...
    >
      <div>
        ...
        <p className="mt-1 text-sm text-white/45">Manage your plan, payment methods, and invoices.</p>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
        <div className="flex items-start justify-between gap-4">
          ...
          <Link
            ...
            className="rounded-xl bg-primary text-black ring-1 ring-inset ring-black/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.45),0_1px_2px_rgba(0,0,0,0.35)] px-4 py-2.5 text-sm font-bold transition hover:brightness-105 active:scale-[0.99]"
          >
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            type="button"
            className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 hover:text-foreground px-3 py-1.5 text-xs font-semibold text-white/70 transition"
          >
        ...
        {paymentMethods.length === 0 ? (
          <div className="rounded-xl border border-dashed border-white/10 bg-white/[0.02] p-6 text-center">
            <FontAwesomeIcon icon={faCreditCard} className="text-white/30" style={{ fontSize: 22 }} />
            <p className="mt-3 text-sm font-medium text-foreground">No payment method on file</p>
            <p className="mt-1 text-xs text-white/45">Add a card to top up credits or upgrade your plan.</p>
          </div>
        ) : (
          <ul className="space-y-2">
            {paymentMethods.map((pm) => (
              ...
              >
                <span className="flex h-9 w-12 items-center justify-center rounded-md border border-white/10 bg-white/5 text-[10px] font-bold uppercase tracking-widest text-white/70">
                  {pm.brand}
                </span>
                <div className="flex-1">
                  <p className="text-sm font-medium text-foreground">•••• {pm.last4}</p>
                  <p className="text-[11px] text-white/45">Expires {pm.exp}</p>
                </div>
                <button
                  type="button"
                  className="text-xs text-white/55 hover:text-foreground transition"
                >
                  Remove
                </button>
              </li>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-white/50">Receipts and invoices are sent here.</p>
          </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link
            href="/credits"
            className="text-[11px] text-white/55 hover:text-foreground transition flex items-center gap-1"
          >
            View all
        ...
        {invoices.length === 0 ? (
          <p className="rounded-xl border border-white/10 bg-white/[0.02] px-4 py-5 text-center text-sm text-white/55">
            No invoices yet.
          </p>
        ) : (
          <ul className="divide-y divide-white/5">
            {invoices.map((inv) => (
              <li key={inv.id} className="flex items-center gap-3 py-3">
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/5 border border-white/10 text-white/60">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/45">{inv.date}</p>
                </div>
                <span
                  className={cn(
                ...
                <button
                  type="button"
                  className="ml-1 flex h-7 w-7 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-white/55 hover:text-foreground hover:bg-white/10 transition"
      ...
      {isPaid && (
        <div className="rounded-2xl border border-destructive/25 bg-destructive/[0.04] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
          <div className="flex items-start justify-between gap-4">
            ...
            {cancelled ? (
              <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest text-white/60">
                Cancelled
              </span>
            ) : confirmCancel ? (
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  type="button"
                  onClick={onCancel}
                  className="rounded-xl bg-destructive text-white px-3 py-2 text-xs font-bold transition hover:brightness-110 active:scale-[0.99]"
                >
                  Confirm cancel
                </button>
              </div>
