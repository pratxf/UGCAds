...
export default function AdminAvatarsPage() {
  ...
  async function handleDeleteCategory(id: string) {
    if (!confirm("Delete this category? Avatars in it will be uncategorized.")) return;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Avatar Library</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage UGC avatars shown to users
          </p>
        </div>
        ...
        {categories.map((c) => (
          <CategoryChip key={c.id} active={filter === c.id} onClick={() => setFilter(c.id)}>
            ...
            <span className="opacity-50 ml-1">
              {avatars.filter((a) => a.categoryId === c.id).length}
            </span>
          </CategoryChip>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border p-12 text-center text-muted-foreground">
          No avatars yet. Click <strong>Upload Avatar</strong> to add one.
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {filtered.map((a) => (
            <div
              key={a.id}
              className={cn(
                "group relative aspect-square rounded-2xl border bg-card overflow-hidden",
            ...
            >
              ...
              <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition">
                ...
                <button
                  ...
                  className="flex size-7 items-center justify-center rounded-lg bg-black/60 backdrop-blur text-white/80 hover:bg-destructive transition"
                >
      ...
      <div className="space-y-4">
        ...
        >
          {preview ? (
            ...
            <div className="text-center text-muted-foreground text-sm">Click to upload image</div>
          )}
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-md rounded-3xl border border-border bg-[#0c0c10] shadow-2xl overflow-hidden"
      >
